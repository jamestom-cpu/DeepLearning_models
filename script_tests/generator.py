...
class RandomMagneticDipoleGenerator():
    def __init__(
            self, resolution, 
            xbounds, ybounds, 
            dipole_height, 
    ...
    def _return_magnetic_dipole_array(self):
        r0 = self._return_r0()
        orientations = self._return_orientation()
        M = self._generate_random_moments()
    ...
    def generate_labeled_Hfields(self):
        Hx, Hy = self.generate_random_H_fields()
        fields = np.stack((Hx.scan, Hy.scan), axis=0)
        return fields, self.mask
    
    def plot_labeled_data(self, fields, mask, FIGSIZE=(10,3), image_folder="images", savename="random_dipole_field.png"):
...
if __name__ == "__main__":
    ...
    field, target = rdg.generate_labeled_Hfields()
